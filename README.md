...
- **官方浏览器登录（默认）**：保留原有流程，在账号独立 Chrome 环境中完成官方登录与 Codex OAuth。
- **协议登录与授权（实验）**：参考 toSub2 的交互式协议登录流程，在后台连续完成 ChatGPT 网页登录和 Codex OAuth。需要邮箱验证码、密码、TOTP、手机号或短信验证码时，对应账号卡片会显示专用输入框与确认按钮；输入只传给当前登录进程，不写入配置、日志或历史记录。
...
- 如果官方 Codex OAuth 明确要求绑定手机号，Navo 会参考 `codex-auth-helper` 的网页会话导入思路，从该账号独立 Chrome 读取当前网页访问凭证、生成临时 Codex 授权，并先调用 Codex 额度接口验证；验证通过才写入账号池，失败则继续显示手机号输入流程。
- 网页会话不包含官方 Codex OAuth refresh token，因此该路径是手机号绑定场景的回退机制；标准路径仍优先使用官方 Codex OAuth。网页访问凭证失效后，需要从仍保持登录的独立 Chrome 重新同步或重新执行官方授权。
...
- 网页登录成功后，协议流程会继续执行 Codex OAuth。若服务端要求手机号或短信验证码，账号卡片会切换为对应输入步骤；OAuth 凭证与网页会话都完成后账号才会正式入池。
...
“授权健康与迁移”工具中提供独立的公网邮箱接口检测入口，用于检查 HTTP 状态、响应格式、大小和脱敏后的结构记录。接口地址与鉴权参数不会写入配置；检测结果不包含邮件正文、主题、验证码、Cookie 或令牌，也不会连接协议登录输入通道。
