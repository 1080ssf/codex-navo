...
- 使用单个 `.codexnavo` 文件迁移 Codex 授权（不包含 Chrome 登录状态）
...
- “账号健康”会检查本地授权文件，并通过 Codex 官方链路验证仍可使用；授权中断后可以继续或取消。
- “导出授权”会生成单个 `.codexnavo` 文件，无需密码或配套密钥文件。
- 授权包内部包含版本清单、账号显示信息、去重指纹和完整的官方 Codex `auth.json`；不会额外收集其他应用的访问令牌。
- “导入授权”会检查文件完整性、识别重复账号并尝试在线验证；明确失效的授权不会写入账号池。
- 授权包只恢复 Codex 授权，不恢复 Chrome Cookie、密码、网页历史或已打开标签页。导入后网页端仍需单独登录。

`.codexnavo` 是未加密的高度敏感文件，包含可用的 Codex 登录令牌。任何拿到该文件的人都可能获得对应账号权限，请仅通过可信渠道传输，并在迁移完成后删除不再需要的副本。
...
上述路径均已加入 `.gitignore`。不要将它们上传到 Git、网盘、Issue、聊天工具或公开截图中。只有用户主动执行“导出授权”时，工具才会生成包含 Codex 令牌的 `.codexnavo` 文件；工具不会导出 Cookie 或密码，也不会绕过密码、验证码、安全检查或服务端风控。
...
- 授权包能迁移 Codex 授权，但服务端仍可能根据安全策略要求重新验证；Chrome 网页登录状态不能迁移。
- Windows Chrome Cookie 受 DPAPI/App-Bound Encryption 的机器与应用身份保护，复制浏览器数据库不能可靠跨设备继续登录。
