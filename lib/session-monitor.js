  ...
  apply(record) {
    ...
    if (record?.type === 'event_msg') {
      switch (payload.type) {
        ...
        case 'token_count': {
          const raw = payload.info?.last_token_usage || payload.info?.total_token_usage || payload.info || {};
          this.value.usage = normalizeUsage(raw);
...
module.exports = { CodexSessionMonitor, SessionState, normalizeUsage, compactText, DEFAULT_ACTIVE_WINDOW_MS };
