...
class SessionState {
  constructor(file) {
    this.file = file;
    this.fileSessionId = sessionIdFromFile(file);
    this.lastFileActivityAt = 0;
    this.sessionUsageTotal = null;
    this.taskUsageBaseline = null;
    this.hasPrimaryMeta = false;
    this.value = {
      id: this.fileSessionId || path.basename(file, '.jsonl'),
      threadName: '',
      project: '未知项目',
      cwd: '',
      originator: 'Codex',
  ...
  setMeta(payload = {}) {
    ...
    if (!this.fileSessionId || payloadId === this.fileSessionId) this.value.id = payloadId || this.value.id;
    ...
    if (payload.source !== undefined || payload.parent_thread_id !== undefined) {
      const source = typeof payload.source === 'string' ? (() => { try { return JSON.parse(payload.source); } catch { return payload.source; } })() : payload.source;
      const spawn = source?.subagent?.thread_spawn;
      const threadSource = String(payload.thread_source || '').toLowerCase();
      this.value.isSubagent = this.value.isSubagent || Boolean(spawn || threadSource === 'subagent');
      this.value.parentThreadId = String(spawn?.parent_thread_id || payload.parent_thread_id || this.value.parentThreadId || '');
    }
    this.value.originator = payload.originator || (typeof payload.source === 'string' ? payload.source : '') || this.value.originator;
  }
  ...
  apply(record) {
    ...
    if (record?.type === 'session_meta' && !this.hasPrimaryMeta) {
      this.setMeta(payload);
      this.hasPrimaryMeta = true;
    }
    ...
    if (terminal) terminal.task.lastUpdatedAt = at;
    return terminal;
  }
...
class CodexSessionMonitor extends EventEmitter {
  ...
  async start() {
    ...
    this.running = true;
    // Visibility can change later when a launch-scoped catalog is restored.
    // A first read of such a file is not evidence of a newly completed task.
    this.monitorStartedAt = this.now();
  ...
  refreshCatalog() {
    ...
    try {
      ...
      const columns = new Set(database.prepare('PRAGMA table_info(threads)').all().map((item) => item.name));
      const lineage = `${columns.has('thread_source') ? 'thread_source' : "'' AS thread_source"},${columns.has('source') ? 'source' : "'' AS source"}`;
      const rows = database.prepare(`SELECT id,cwd,title,name,first_user_message,model,model_provider,archived,rollout_path,${lineage},COALESCE(NULLIF(recency_at_ms,0),NULLIF(updated_at_ms,0),updated_at*1000) AS updated_ms FROM threads ORDER BY updated_ms DESC`).all();
  ...
  async readFile(entry, initial) {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      try {
        ...
        const terminal = parser.apply(record);
        if (terminal && this.monitorStartedAt > 0) {
          const recordedAt = timestamp(record.timestamp, '');
          const completedAt = terminal.type === 'completed'
            ? timestamp(record.payload?.completed_at, recordedAt) : recordedAt;
          const eventTime = Date.parse(completedAt);
          // Use recorded event time, never the discovery time or file mtime.
          // This also permits a genuinely new completion during a slow initial
          // scan, and does not fabricate fresh notifications for undated logs.
          if (Number.isFinite(eventTime) && eventTime >= this.monitorStartedAt
            && !terminal.task.isSubagent && !terminal.task.parentThreadId) this.emit('terminal', terminal);
        }
      } catch {}
    }
    this.applyCatalog(parser);
  ...
  applyCatalog(parser) {
    ...
    if (!item) return;
    parser.setMeta({ cwd: item.cwd, thread_source: item.thread_source, source: item.source });
