...
const { readSubscriptionInPage, subscriptionReadError } = require('./subscription-read');
...
async function readProtocolSubscription({
  ...
  try {
    let value = null;
    const retries = Math.max(1, Math.min(2, Math.round(Number(attempts) || 1)));
    ...
    for (let attempt = 1; attempt <= retries; attempt += 1) {
      ...
      const evaluation = await cdpCommand(socket, 'Runtime.evaluate', {
        expression: `(${readSubscriptionInPage.toString()})(${JSON.stringify({ accessToken: suppliedAccessToken, accountId: suppliedAccountId })})`,
        awaitPromise: true,
        returnByValue: true,
      }, 20_000);
      value = evaluation?.result?.value || null;
      if (value?.ok) break;
      if (value?.code !== 'network_error' && !(value?.status >= 500)) break;
    }
    if (!value?.ok) throw subscriptionReadError(value);
...
async function injectProtocolCookies({
  ...
  try {
    ...
    // Flush imported cookies even when navigation or verification fails.
    if (closeBrowser) await closeChromeAndWait(socket);
    try { socket.close(); } catch {}
  }
}
