...
async function injectProtocolCookies({
  ...
  const target = targets.find((item) => item.type === 'page' && /^https:\/\/(?:chatgpt\.com|auth\.openai\.com)\//i.test(item.url))
  ...
  try {
    ...
    await cdpCommand(socket, 'Page.navigate', { url: 'https://chatgpt.com/' });
    let value = {};
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      if (value.ok && /\"user\"\s*:|\"accessToken\"\s*:/i.test(String(value.text || ''))) break;
    }
    const verified = Boolean(value.ok && /\"user\"\s*:|\"accessToken\"\s*:/i.test(String(value.text || '')));
