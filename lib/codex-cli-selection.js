...
async function discoverCliCandidates({ environment = process.env, configured = '', desktopExecutable = '', managedRoot = '', desktopCandidates = installedDesktopCli,
  ...
  if (managedRoot) schedule('navo', managedRoot, async context => {
    const entries = await context.read(() => readdir(managedRoot, { withFileTypes: true }), managedRoot) || [];
    for (const entry of entries) {
      if (!context.active()) return;
      if (entry.isDirectory() && /^stable-\d+\.\d+\.\d+-[\w-]+$/.test(entry.name)
        && (await context.read(() => stat(path.join(managedRoot, entry.name, 'ready.json')), entry.name))?.isFile()) {
        await context.probe(path.join(managedRoot, entry.name, 'codex.exe'));
      }
    }
  });
...
const reasonLabels = {
  ...
  protocol_failed: ['app-server 协议验证失败', 'App-server handshake failed'],
  protocol_timeout: ['app-server 协议验证超时', 'App-server handshake timed out'],
};
...
function createCliResolver({ discover = discoverCliCandidates, run = runHidden, stat = file => fs.promises.stat(file), now = Date.now, timeoutMs = 8000, totalTimeoutMs = 24000, statTimeoutMs = 2000,
  discoveryTimeoutMs = 8000, sourceTimeoutMs = 3000, discoveryConcurrency = 4, probeProtocol = probeCliProtocol } = {}) {
  const cache = new Map();
  ...
  async function inspect(options = {}) {
    const identity = JSON.stringify([options.configured || '', options.desktopExecutable || '', options.managedRoot || '']);
    if (pending) {
    ...
    pending = (async () => {
      ...
      let eligible = rows.filter(row => row.status === 'ready' && row.stable !== false && (!options.configured || row.explicit));
      // Prefer any working stable build. Probe a preview only when no stable build is available.
      if (!eligible.length) {
        const previews = rows.filter(row => row.status === 'prerelease' || (row.status === 'ready' && row.stable === false))
          .sort((a, b) => b.parts[0] - a.parts[0] || b.parts[1] - a.parts[1] || b.parts[2] - a.parts[2]);
        for (const row of previews) {
          if (row.status !== 'ready') {
            const remaining = totalTimeoutMs - (now() - started);
            const check = remaining > 0 ? await probeProtocol(row.path, Math.min(timeoutMs, remaining))
              .catch(() => ({ ok: false, code: 'protocol_failed' })) : { ok: false, code: 'budget_exhausted' };
            row.status = check.ok ? 'ready' : check.code || 'protocol_failed';
            row.protocolVerified = check.ok === true;
            row.protocolDurationMs = check.durationMs || 0;
            if (check.processErrorCode) row.processErrorCode = check.processErrorCode;
            const cached = cache.get(row.path);
            if (cached) {
              cached.result = { ...cached.result, status: row.status, protocolVerified: row.protocolVerified,
                protocolDurationMs: row.protocolDurationMs, processErrorCode: row.processErrorCode };
              cached.until = now() + (check.ok ? 60000 : 15000);
            }
          }
          if (row.status === 'ready') { eligible = [row]; break; }
        }
      }
      let selected = null;
      if (eligible.length) {
