...
function publicSource(source) {
  ...
  const statusRank = (node) => {
    if (node.status === 'available' && node.delay != null) return 0;
    if (node.status === 'unsupported-region' && node.delay != null) return 1;
    if (node.delay != null) return 2;
  ...
  nodes.sort((left, right) => {
    ...
    if (rank) return rank;
    const delay = (left.delay ?? Number.POSITIVE_INFINITY) - (right.delay ?? Number.POSITIVE_INFINITY);
...
function selectTaskRoute(data, preferredAccountId = '') {
  ...
  const resolveAssignment = (accountId) => {
    ...
    if (!source || !node || node.status !== 'available' || !Number.isFinite(Number(node.delay))) return null;
    return { accountId, sourceId: source.id, nodeName: node.name, delay: Number(node.delay) };
  };
  ...
  return sources
    .flatMap((source) => (source.nodes || [])
      .filter((node) => node.status === 'available' && Number.isFinite(Number(node.delay)))
      .map((node) => ({ accountId: '', sourceId: source.id, nodeName: node.name, delay: Number(node.delay) })))
...
function probeChatGpt(mixedPort, timeoutMs = 12_000) {
  return new Promise((resolve, reject) => {
    ...
    socket.on('data', (chunk) => {
      ...
      secure.once('secureConnect', () => {
        stage = 'http';
        secure.write('GET /backend-api/models HTTP/1.1\r\nHost: chatgpt.com\r\nUser-Agent: Mozilla/5.0 Codex-Navo/1.0\r\nAccept: application/json,text/plain,*/*\r\nAccept-Encoding: identity\r\nConnection: close\r\n\r\n');
      });
      secure.on('data', (data) => {
        response = Buffer.concat([response, data]);
        if (response.length > 256 * 1024) secure.destroy();
      ...
      secure.once('close', () => {
        ...
        return finish(null, classifyChatGptResponse(text, Date.now() - startedAt));
      });
    });
    socket.once('error', (error) => finish(error));
  });
...
class AccountNetworkManager {
  ...
  publicAssignment(accountId, value = this.data.assignments[accountId]) {
    ...
    return {
      ...
      label: source ? `${source.name} · ${value.nodeName}` : value.nodeName,
  ...
  async testNode(sourceId, nodeName) {
    ...
    try {
      ...
      const result = await probeOpenAiRoute(runtime.mixedPort);
      const delay = result.latencyMs;
      Object.assign(found, { delay, status: result.status, checkedAt: new Date().toISOString() });
      source.updatedAt = new Date().toISOString();
      this.save();
      this.stopTask();
      return { ...result, delay };
    } catch (error) {
      Object.assign(found, { delay: null, status: 'connection-failed', checkedAt: new Date().toISOString() });
      source.updatedAt = new Date().toISOString();
      this.save();
      this.stopTask();
      return {
  ...
  async testSource(sourceId, concurrency = 3) {
    ...
    for (const node of source.nodes) Object.assign(node, { delay: null, status: 'checking', checkedAt: null });
    this.save();
    const publish = () => {
      ...
      try {
        ...
        while (cursor < source.nodes.length) {
          ...
          if (!availableNames.has(node.name)) {
            Object.assign(node, { delay: null, status: 'connection-failed', checkedAt: new Date().toISOString() });
            summary.failed += 1;
            publish();
            continue;
          }
          try {
            ...
            const result = await probeOpenAiRoute(runtime.mixedPort, 10_000);
            Object.assign(node, { delay: result.latencyMs, status: result.status, checkedAt: new Date().toISOString() });
  ...
  async preflightAccount(accountId, timeoutMs = 12_000) {
    ...
    const result = await probeOpenAiRoute(runtime.mixedPort, timeoutMs);
    const source = this.source(assignment.sourceId);
    const node = (source.nodes || []).find((item) => item.name === assignment.nodeName);
    if (node) {
