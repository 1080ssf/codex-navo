...
// Port 18301 is the stable front door used by Codex Desktop. Proxy cores keep
// their independent dynamic endpoints so multiple accounts and API keys can
// still run concurrently without changing the port visible to the sandbox.
const PROXY_RUNTIME_PORT_START = 18302;
...
function queryList(params, ...names) {
  const value = names.map((name) => params.get(name)).find((item) => item);
  return value ? safeDecode(value).split(',').map((item) => item.trim()).filter(Boolean) : undefined;
}

function validPort(value, fallback = 0) {
  const port = Number(value || fallback);
  if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error('节点端口无效');
  return port;
}

function transportOptions(proxy, params) {
  const network = cleanText(params.get('type') || params.get('network') || '', 30).toLowerCase();
  if (network) proxy.network = network;
  if (network === 'ws') {
    proxy['ws-opts'] = {
      path: safeDecode(params.get('path') || '/'),
      ...(params.get('host') ? { headers: { Host: safeDecode(params.get('host')) } } : {}),
    };
  }
  if (network === 'grpc') proxy['grpc-opts'] = { 'grpc-service-name': safeDecode(params.get('serviceName') || params.get('service-name') || '') };
  return proxy;
}

function standardUrlProxy(uri, name, type) {
  const parsed = new URL(uri);
  const params = parsed.searchParams;
  const proxy = {
    name, type, server: parsed.hostname,
    port: validPort(parsed.port, type === 'http' ? (parsed.protocol === 'https:' ? 443 : 80) : 1080),
  };
  const username = safeDecode(parsed.username);
  const password = safeDecode(parsed.password);
  if (username) proxy.username = username;
  if (password) proxy.password = password;
  if (type === 'http' && parsed.protocol === 'https:') proxy.tls = true;
  if (queryBoolean(params, 'skip-cert-verify', 'allowInsecure', 'insecure')) proxy['skip-cert-verify'] = true;
  if (params.get('sni')) proxy.sni = safeDecode(params.get('sni'));
  return proxy;
}

function ssProxy(uri, name) {
  const raw = String(uri).replace(/^ss:\/\//i, '').split('#')[0];
  const withoutQuery = raw.split('?')[0];
  let credentials = '';
  let endpoint = '';
  if (withoutQuery.includes('@')) {
    const separator = withoutQuery.lastIndexOf('@');
    const encodedCredentials = withoutQuery.slice(0, separator);
    credentials = encodedCredentials.includes(':') ? safeDecode(encodedCredentials) : decodeBase64(encodedCredentials);
    endpoint = withoutQuery.slice(separator + 1);
  } else {
    const decoded = decodeBase64(withoutQuery);
    const separator = decoded.lastIndexOf('@');
    if (separator < 0) throw new Error('SS 节点缺少服务器地址');
    credentials = decoded.slice(0, separator);
    endpoint = decoded.slice(separator + 1);
  }
  const split = credentials.indexOf(':');
  if (split < 1) throw new Error('SS 节点缺少加密方式或密码');
  const target = new URL(`http://${endpoint}`);
  const proxy = {
    name, type: 'ss', server: target.hostname, port: validPort(target.port),
    cipher: credentials.slice(0, split), password: credentials.slice(split + 1),
  };
  const params = new URL(uri).searchParams;
  const plugin = safeDecode(params.get('plugin') || '');
  if (plugin) {
...
function ssrProxy(uri, name) {
  const decoded = decodeBase64(String(uri).replace(/^ssr:\/\//i, '').split('#')[0]);
  const [main, query = ''] = decoded.split('/?');
  const parts = main.split(':');
  if (parts.length < 6) throw new Error('SSR 节点字段不完整');
  const [server, port, protocol, cipher, obfs, ...passwordParts] = parts;
  const params = new URLSearchParams(query);
  const proxy = {
    name, type: 'ssr', server, port: validPort(port), protocol, cipher, obfs,
    password: decodeBase64(passwordParts.join(':')),
  };
  for (const [queryName, targetName] of [['obfsparam', 'obfs-param'], ['protoparam', 'protocol-param']]) {
    const value = decodeBase64(params.get(queryName));
    if (value) proxy[targetName] = value;
  }
  return proxy;
}

function vmessProxy(uri, name) {
  const value = JSON.parse(decodeBase64(String(uri).replace(/^vmess:\/\//i, '').split('#')[0]));
  const proxy = {
    name, type: 'vmess', server: cleanText(value.add, 255), port: validPort(value.port), uuid: cleanText(value.id, 100),
    alterId: Number(value.aid) || 0, cipher: cleanText(value.scy || value.security || 'auto', 40) || 'auto',
  };
  const network = cleanText(value.net, 30).toLowerCase();
  if (network) proxy.network = network;
  if (String(value.tls || '').toLowerCase() === 'tls') proxy.tls = true;
  if (value.sni) proxy.servername = cleanText(value.sni, 255);
  if (value.alpn) proxy.alpn = String(value.alpn).split(',').filter(Boolean);
  if (String(value.allowInsecure) === '1' || value.allowInsecure === true) proxy['skip-cert-verify'] = true;
  if (network === 'ws') proxy['ws-opts'] = { path: value.path || '/', ...(value.host ? { headers: { Host: value.host } } : {}) };
  if (network === 'grpc') proxy['grpc-opts'] = { 'grpc-service-name': value.path || value.serviceName || '' };
  return proxy;
}

function vlessOrTrojanProxy(uri, name, type) {
  const parsed = new URL(uri);
  const params = parsed.searchParams;
  const proxy = {
    name, type, server: parsed.hostname, port: validPort(parsed.port, 443),
    ...(type === 'vless' ? { uuid: safeDecode(parsed.username) } : { password: safeDecode(parsed.username || parsed.password) }),
  };
  const security = cleanText(params.get('security'), 30).toLowerCase();
  if (security === 'tls' || security === 'reality' || type === 'trojan') proxy.tls = true;
  if (params.get('sni')) proxy.servername = safeDecode(params.get('sni'));
  if (params.get('flow')) proxy.flow = safeDecode(params.get('flow'));
  if (params.get('fp')) proxy['client-fingerprint'] = safeDecode(params.get('fp'));
  if (queryBoolean(params, 'allowInsecure', 'insecure', 'skip-cert-verify')) proxy['skip-cert-verify'] = true;
  if (security === 'reality') {
    proxy['reality-opts'] = {
      'public-key': safeDecode(params.get('pbk') || params.get('public-key') || ''),
      'short-id': safeDecode(params.get('sid') || params.get('short-id') || ''),
    };
  }
  return transportOptions(proxy, params);
}

function hysteriaProxy(uri, name, version = 2) {
  const parsed = new URL(uri);
  const params = parsed.searchParams;
  const auth = safeDecode(parsed.username || parsed.password);
  const port = validPort(parsed.port, 443);
  const proxy = {
    name, type: version === 2 ? 'hysteria2' : 'hysteria', server: parsed.hostname,
    port,
  };
  if (version === 2) proxy.password = auth;
  else proxy['auth-str'] = safeDecode(params.get('auth') || params.get('auth-str') || auth);
  const sni = params.get('sni') || params.get('peer');
  if (sni) proxy.sni = safeDecode(sni);
  if (queryBoolean(params, 'insecure', 'allowInsecure', 'skip-cert-verify')) proxy['skip-cert-verify'] = true;
  const alpn = queryList(params, 'alpn');
  if (alpn) proxy.alpn = alpn;
  if (version === 2 && params.get('obfs')) proxy.obfs = safeDecode(params.get('obfs'));
  if (version === 2 && (params.get('obfs-password') || params.get('obfsPassword'))) proxy['obfs-password'] = safeDecode(params.get('obfs-password') || params.get('obfsPassword'));
  if (version === 1) {
    proxy.protocol = safeDecode(params.get('protocol') || 'udp');
    proxy.up = safeDecode(params.get('upmbps') || params.get('up') || '50 Mbps');
    proxy.down = safeDecode(params.get('downmbps') || params.get('down') || '200 Mbps');
    if (params.get('obfs')) proxy.obfs = safeDecode(params.get('obfs'));
  }
  return proxy;
}

function tuicProxy(uri, name) {
  const parsed = new URL(uri);
  const params = parsed.searchParams;
  const proxy = {
    name, type: 'tuic', server: parsed.hostname, port: validPort(parsed.port, 443),
    uuid: safeDecode(parsed.username), password: safeDecode(parsed.password),
  };
  proxy.udp = !params.has('udp') || queryBoolean(params, 'udp');
  if (params.get('sni')) proxy.sni = safeDecode(params.get('sni'));
  if (params.get('congestion_control') || params.get('congestion-controller')) proxy['congestion-controller'] = safeDecode(params.get('congestion_control') || params.get('congestion-controller'));
  if (params.get('udp_relay_mode') || params.get('udp-relay-mode')) proxy['udp-relay-mode'] = safeDecode(params.get('udp_relay_mode') || params.get('udp-relay-mode'));
  if (queryBoolean(params, 'allowInsecure', 'insecure', 'skip-cert-verify')) proxy['skip-cert-verify'] = true;
  const alpn = queryList(params, 'alpn');
  if (alpn) proxy.alpn = alpn;
  return proxy;
...
function wireguardProxy(uri, name) {
  const parsed = new URL(uri);
  const params = parsed.searchParams;
  const proxy = {
    name, type: 'wireguard', server: parsed.hostname, port: validPort(parsed.port, 51820),
    ip: safeDecode(params.get('ip') || params.get('address') || '172.16.0.2'),
    'private-key': safeDecode(params.get('privatekey') || params.get('private-key') || parsed.username),
    'public-key': safeDecode(params.get('publickey') || params.get('public-key') || ''),
  };
  proxy.udp = !params.has('udp') || queryBoolean(params, 'udp');
  if (params.get('mtu')) proxy.mtu = Number(params.get('mtu'));
  if (params.get('reserved')) proxy.reserved = params.get('reserved').split(',').map(Number);
  return proxy;
}

function proxyUriToMihomo(uri, name = '') {
  const protocol = String(uri).match(/^([a-z][a-z0-9+.-]*):\/\//i)?.[1]?.toLowerCase();
  const nodeName = cleanText(name || nodeNameFromUri(uri, protocol, 0), 100);
  if (protocol === 'http' || protocol === 'https') return standardUrlProxy(uri, nodeName, 'http');
  if (protocol === 'socks' || protocol === 'socks5') return standardUrlProxy(uri, nodeName, 'socks5');
  if (protocol === 'ss') return ssProxy(uri, nodeName);
  if (protocol === 'ssr') return ssrProxy(uri, nodeName);
  if (protocol === 'vmess') return vmessProxy(uri, nodeName);
  if (protocol === 'vless' || protocol === 'trojan') return vlessOrTrojanProxy(uri, nodeName, protocol);
  if (protocol === 'hysteria') return hysteriaProxy(uri, nodeName, 1);
  if (protocol === 'hysteria2' || protocol === 'hy2') return hysteriaProxy(uri, nodeName, 2);
  if (protocol === 'tuic') return tuicProxy(uri, nodeName);
  if (protocol === 'wireguard') return wireguardProxy(uri, nodeName);
  throw new Error(`不支持的代理协议：${protocol || 'unknown'}`);
}

function hydrateUriNodes(source) {
  const uris = proxyUrisFromText(source?.content || '');
  const existing = Array.isArray(source?.nodes) ? source.nodes : [];
  return uris.map((uri, index) => {
    const protocol = uri.match(/^([a-z][a-z0-9+.-]*):\/\//i)?.[1]?.toLowerCase();
    const inferredName = nodeNameFromUri(uri, protocol, index);
    const previous = existing.find((node) => node.uri === uri)
      || existing.find((node) => node.name === inferredName)
      || existing[index]
      || {};
    return {
      ...previous,
      name: cleanText(previous.name || inferredName, 160),
...
function parseProxyInput(input, requestedName = '') {
  ...
  for (const line of uriLines) {
    ...
    const name = nodeNameFromUri(line, protocol, nodes.length);
    try { proxyUriToMihomo(line, name); }
    catch (error) { throw new Error(`${protocolLabel(protocol)} 节点格式无效：${error.message}`); }
    nodes.push({
      name,
      protocol: protocolLabel(protocol),
      uri: line,
...
async function freeProxyPort(preferredPort) {
  ...
  if (Number.isInteger(preferred) && preferred >= PROXY_RUNTIME_PORT_START && preferred <= PROXY_PORT_END
    && await canListen(preferred)) return preferred;
  for (let port = PROXY_RUNTIME_PORT_START; port <= PROXY_PORT_END; port += 1) {
...
class AccountNetworkManager {
  constructor({ runtimeRoot, audit = () => {} }) {
    ...
    let repairedUriSources = false;
    for (const source of this.data.sources) {
      if (source?.kind !== 'local' || source?.format !== 'uri-list') continue;
      const hydrated = hydrateUriNodes(source);
      if (hydrated.length && (hydrated.length !== (source.nodes || []).length || (source.nodes || []).some((node) => !node.uri))) {
        source.nodes = hydrated;
        repairedUriSources = true;
      }
    }
    if (repairedUriSources) writeJsonAtomic(this.dataFile, this.data);
    this.inspector = null;
  ...
  writeProvider(source, runtimeDir) {
    ...
    const content = source.format === 'uri-list'
      ? `${JSON.stringify({ proxies: hydrateUriNodes(source).map((node) => proxyUriToMihomo(node.uri, node.name)) }, null, 2)}\n`
      : source.content;
  ...
  async refreshSource(sourceId) {
    ...
    try {
      ...
      const providerNodes = await this.waitForProviderNodes(runtime, source, { refresh: true });
      const uriNodes = source.format === 'uri-list' ? hydrateUriNodes(source) : [];
      source.nodes = providerNodes.map((node, index) => {
        if (!uriNodes.length) return node;
        const original = uriNodes.find((item) => item.name === node.name) || uriNodes[index];
        return original ? { ...original, ...node, uri: original.uri } : node;
      });
  ...
  async ensureTask(preferredAccountId = '') {
    // Measurements (including temporary "checking" statuses) are not a route
    // configuration change. Preserve the endpoint held by active downloads and
    // quota requests; source/assignment mutations explicitly call stopTask().
    if (this.taskRuntime && processAlive(this.taskRuntime.child)) return this.taskRuntime;
    ...
    this.stopTask();
    const generation = this.taskRuntimeGeneration;
    const pending = (async () => {
      const source = this.source(selected.sourceId);
      const runtime = await this.startRuntime('background-tasks', [source], 'Navo Tasks');
      try {
        if (generation !== this.taskRuntimeGeneration) throw new Error('后台任务代理配置已变化，请重试');
        const availableNodes = await this.waitForProviderNodes(runtime, source);
      ...
      if (this.taskRuntimePromise === pending) this.taskRuntimePromise = null;
    });
    this.taskRuntimePromise = pending;
    return pending;
  }
