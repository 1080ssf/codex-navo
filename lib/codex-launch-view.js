...
function restoreLaunchView(record) {
  ...
  try {
    if (record.globalBackup && record.globalFile && fs.existsSync(record.globalBackup)) {
      ...
      writeJsonAtomic(record.globalFile, mergeLaunchGlobalState(original, live, record.threadIds, record.projectIds));
    }
...
function mergeLaunchGlobalState(original, live, selectedIds = [], selectedProjectIds = []) {
  ...
  const selectedProjects = new Set(selectedProjectIds);
  ...
  for (const projectId of selectedProjects) {
    if (!liveVisibleProjects.has(projectId) || !liveProjects[projectId]) {
      ...
      output['local-projects'][projectId] = liveProjects[projectId];
    }
  }
  for (const projectId of liveProjectOrder) {
    if (liveProjects[projectId] && !(original?.['local-projects'] || {})[projectId]) {
      output['local-projects'][projectId] = liveProjects[projectId];
    }
  }
  output['project-order'] = [
    ...(output['project-order'] || []).filter((id) => !selectedProjects.has(id)),
    ...liveProjectOrder,
  ].filter((id, index, items) => output['local-projects'][id] && items.indexOf(id) === index);
  output['pinned-project-ids'] = [
    ...(output['pinned-project-ids'] || []).filter((id) => !selectedProjects.has(id)),
    ...(live?.['pinned-project-ids'] || []),
  ].filter((id, index, items) => output['local-projects'][id] && items.indexOf(id) === index);

  output['thread-project-assignments'] = { ...(output['thread-project-assignments'] || {}) };
  for (const id of selected) delete output['thread-project-assignments'][id];
  for (const [id, value] of Object.entries(live?.['thread-project-assignments'] || {})) {
    const originalValue = original?.['thread-project-assignments']?.[id];
    if (selected.has(id)) output['thread-project-assignments'][id] = originalValue || value;
    else if (!originalValue) output['thread-project-assignments'][id] = value;
  }
  output['projectless-thread-ids'] = [...new Set([
    ...(output['projectless-thread-ids'] || []).filter((id) => !selected.has(id)),
    ...(live?.['projectless-thread-ids'] || []).filter((id) => selected.has(id)),
  ])];
  output['sidebar-project-thread-orders'] = { ...(output['sidebar-project-thread-orders'] || {}) };
  for (const projectId of selectedProjects) delete output['sidebar-project-thread-orders'][projectId];
  for (const [projectId, value] of Object.entries(live?.['sidebar-project-thread-orders'] || {})) {
    if (!(original?.['sidebar-project-thread-orders'] || {})[projectId]) output['sidebar-project-thread-orders'][projectId] = value;
  }
  ...
  for (const [key, value] of Object.entries(live?.['electron-persisted-atom-state'] || {})) {
    ...
    if (!match || selected.has(match[1])) output['electron-persisted-atom-state'][key] = value;
  }
  if (Object.prototype.hasOwnProperty.call(live || {}, 'selected-project')) output['selected-project'] = live['selected-project'];
  return output;
}
...
async function optimizeRolloutFile(file, backupRoot, options = {}) {
  ...
  if (!checkpoint || checkpoint.segmentStartLine === checkpoint.lineNumber
    || !inspection.sessionMetaLines.size || inspection.invalidLines || inspection.compactedCount < 1) return null;
  // A rollback newer than the checkpoint can make an older checkpoint live again.
  // Skip instead of guessing which historical segment Codex will reconstruct.
  ...
  try {
    ...
    if (await hashFile(temporary) !== expectedDigest.digest('hex')) throw new Error('Optimized rollout checksum mismatch');
...
async function restoreRolloutBackup(codexHome, backupRoot, backupId) {
  ...
  const temporary = `${source}.navo-restore-${process.pid}-${Date.now()}.tmp`;
  ...
  try {
    ...
    if (fs.existsSync(source)) {
      const safety = path.join(backupRoot, `${path.basename(source)}.pre-restore-${Date.now()}.bak`);
      fs.copyFileSync(source, safety);
    ...
    return { restored: source, bytes: fs.statSync(source).size };
  } catch (error) {
    fs.rmSync(temporary, { force: true });
    throw error;
  }
