...
function runPython(script, args, timeout = 30_000, input) {
  for (const executable of ['python', 'py']) {
    const commandArgs = executable === 'py' ? ['-3', '-c', script, ...args] : ['-c', script, ...args];
    const result = spawnSync(executable, commandArgs, {
      encoding: 'utf8', windowsHide: true, timeout, input, maxBuffer: 8 * 1024 * 1024,
    });
...
function runPythonWithLockRetry(script, args, attempts = 12, input) {
  let result = null;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    result = runPython(script, args, 30_000, input);
...
function syncSessionIndexNames(codexHome) {
  ...
  const result = runPythonWithLockRetry(SYNC_SESSION_INDEX_NAMES_SCRIPT, [codexHome], 2, JSON.stringify(Object.fromEntries(names)));
  if (result.status !== 0) throw new Error(String(result.stderr || result.stdout || result.error?.code || 'Failed to sync Codex conversation names').trim());
