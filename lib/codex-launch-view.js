...
function withDesktopLocale(source, language) {
  ...
  return setTomlSectionValue(source, 'desktop', 'localeOverride', JSON.stringify(locale));
}
...
db_path, backup_path, live_path, ids_json, restore_provider = sys.argv[1:6]
...
try:
    columns = [row[1] for row in dest.execute('PRAGMA table_info(threads)')]
    ...
    with dest:
        for row in source.execute('SELECT * FROM threads'):
            ...
            if restore_provider and 'model_provider' in columns:
                values[columns.index('model_provider')] = restore_provider
  ...
  try {
    ...
    if (fs.existsSync(stateFile)) {
      ...
      if (fs.existsSync(stateBackup)) {
        ...
        record.restoreProvider = modelProvider ? 'openai' : '';
      }
...
function restoreLaunchView(record) {
  ...
  try {
    if (record.stateBackup && record.stateFile && fs.existsSync(record.stateBackup)) {
      ...
      const result = runPython(RESTORE_STATE_SCRIPT, [record.stateFile, record.stateBackup, livePath, JSON.stringify(record.threadIds || []), record.restoreProvider || '']);
