...
const { EXPLICIT_ACCOUNT_SCOPE, normalizeAccountIds, normalizeApiKeyAccountScope } = require('./api-key-members');
...
function normalizeKeyRecord(value = {}, previous = {}) {
  ...
  return {
    ...
    ...normalizeApiKeyAccountScope({
      accountIds: value.accountIds ?? previous.accountIds,
      accountScope: value.accountScope ?? previous.accountScope,
    }),
    showInAccounts: true,
    ...
    usage,
    usageLedger: Array.isArray(previous.usageLedger) ? previous.usageLedger : [],
...
function publicKey(record) {
  const { salt, hash, launchSalt, launchHash, usageLedger, ...safe } = record;
  return safe;
}
...
class ApiServiceManager {
  constructor({ runtimeRoot, writeJsonAtomic, readJson, audit = () => {}, poolForwarder = null, gatewayPort = DEFAULT_GATEWAY_PORT }) {
    ...
    const requiresAccountScopeMigration = storedKeys.some((item) => item && item.hidden !== true
      && normalizeApiKeyAccountScope(item, { migrateLegacy: true }).accountScope !== item.accountScope);
    this.keys = storedKeys
      .filter((item) => item && item.hidden !== true)
      .map((item) => normalizeKeyRecord({ ...item, ...normalizeApiKeyAccountScope(item, { migrateLegacy: true }) }, item));
    if (requiresDailyUsageMigration || requiresAccountScopeMigration) this.saveKeys();
  }
  ...
  ensureAccountPool(models = []) {
    ...
    for (const key of this.keys) {
      ...
      if (key.usage.costVersion < 2) {
        // Current permissions/defaults are not evidence of historical models.
        const estimated = hasHistoricalTokens ? null : 0;
        if (estimated === null) {
        ...
        key.usage.estimatedCostApproximate = false;
  ...
  createKey(value = {}) {
    ...
    if (!accountIds.length) throw Object.assign(new Error('请至少选择一个普通账号或临时账号'), { statusCode: 400 });
    const secret = `sk-navo-${crypto.randomBytes(24).toString('base64url')}`;
    const salt = crypto.randomBytes(16).toString('base64url');
    const record = normalizeKeyRecord({
      ...value,
      accountIds,
      accountScope: EXPLICIT_ACCOUNT_SCOPE,
  ...
  updateKey(id, value = {}) {
    ...
    const hasMemberUpdate = Object.hasOwn(value, 'accountIds');
    const accountIds = hasMemberUpdate ? normalizeAccountIds(value.accountIds) : record.accountIds;
    if (hasMemberUpdate && !accountIds.length) {
      throw Object.assign(new Error('请至少选择一个普通账号或临时账号'), { statusCode: 400 });
    }
    Object.assign(record, normalizeKeyRecord({ ...record, ...value, id, accountIds,
      accountScope: hasMemberUpdate ? EXPLICIT_ACCOUNT_SCOPE : record.accountScope }, record));
    this.saveKeys();
  ...
  recordUsage(record, usage = {}, model = '', usedAt = new Date(), metadata = {}) {
    const requestId = String(metadata.requestId || crypto.randomUUID()).slice(0, 160);
    record.usageLedger ||= [];
    if (record.usageLedger.some((entry) => entry.requestId === requestId)) return false;
    ...
    // Store evidence and aggregates in the SAME atomic snapshot: a separate
    // append log can be ahead/behind keys.json after a crash and double count.
    record.usageLedger.push({ requestId, model: String(model).slice(0, 120), date,
      usedAt: validTimestamp.toISOString(), inputTokens: input, cachedInputTokens: cachedInput,
      cacheWriteInputTokens: cacheWriteInput, outputTokens: output, reasoningOutputTokens: reasoningOutput,
      source: metadata.source === 'model-detection' ? 'model-detection' : 'api', pricingRevision: '2026-09-06' });
    // Bounded evidence retention. Reconciliation never treats a partial ledger
    // as complete: token AND request counts must match each target bucket.
    record.usageLedger = record.usageLedger.filter((entry) => record.dailyUsage.some((day) => day.date === entry.date)).slice(-20000);
    record.updatedAt = validTimestamp.toISOString();
    this.saveKeys();
    return true;
  }

  reconcileHistoricalCosts() {
    ...
    for (const record of this.keys) {
      const ledger = record.usageLedger || [];
      for (const bucket of [{ usage: record.usage }, ...record.dailyUsage]) {
        const entries = ledger.filter((entry) => !bucket.date || entry.date === bucket.date);
        const totals = normalizeUsage();
        for (const entry of entries) {
          totals.requests += 1;
          for (const field of fields) totals[field] += Number(entry[field]) || 0;
          const cost = estimateCost(entry.model, { input: entry.inputTokens, cachedInput: entry.cachedInputTokens,
            cacheWriteInput: entry.cacheWriteInputTokens, output: entry.outputTokens, reasoningOutput: entry.reasoningOutputTokens });
          if (cost === null) totals.unpricedRequests += 1;
          else if (entry.inputTokens || entry.outputTokens) { totals.pricedRequests += 1; totals.estimatedCostUsd += cost; }
        }
        if (!bucket.usage.requests) continue;
        if (totals.requests !== bucket.usage.requests || fields.some((field) => totals[field] !== bucket.usage[field])
          || new Set(entries.map((entry) => entry.requestId)).size !== entries.length) { unavailable += 1; continue; }
        const correction = { estimatedCostUsd: Number(totals.estimatedCostUsd.toFixed(8)), pricedRequests: totals.pricedRequests,
          unpricedRequests: totals.unpricedRequests, estimatedCostApproximate: false, costVersion: 2 };
        if (Object.entries(correction).some(([field, value]) => bucket.usage[field] !== value)) updates.push([bucket.usage, correction]);
      }
    }
    if (updates.length) {
      const backup = `${this.keysFile}.before-ledger-repricing.bak`;
      if (fs.existsSync(this.keysFile) && !fs.existsSync(backup)) fs.copyFileSync(this.keysFile, backup, fs.constants.COPYFILE_EXCL);
      for (const [target, correction] of updates) { Object.assign(target, correction); repaired += 1; }
      this.saveKeys();
    }
