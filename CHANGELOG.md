...
- 授权包支持分片网页登录 Cookie，保留 Cookie 域作用范围；网页验证失败时正常关闭浏览器保存数据，不再删除导入的浏览器环境。
- 网页验证未完成时提供中英文提示，不将未验证状态视为登录成功。过期或被服务端撤销的网页会话仍需重新登录。
- 会话名称同步改用 UTF-8 标准输入，避免 Windows 命令行长度限制；名称同步失败记录诊断并继续启动，不阻塞 Codex。

## 1.2.152

- 按固定线路执行更新：Navo 的 GitHub 检查、下载和重试均走已配置代理；Codex 官方 CDN 检查、下载和重试均显式直连。
- 取消更新自动测速选线，Navo 代理缺失或准备失败时不回退 GitHub 直连；缺少代理提供中英文说明。Codex 官方下载不再等待账号代理准备。
- 保留 1.2.151 的分段下载、断点缓存、有限重试、深色模式和滚轮修复。不改动账号、任务及 Windows 全局代理。

## 1.2.151

- 修复启动前项目选择弹窗的内层列表拦截滚轮问题，保留固定操作栏和单一滚动区域。
- 应用设置恢复浅色、深色、跟随系统，选择后自动保存，启动前应用主题；适配账号、用量、网络、模型检测、重置卡、API Key、更新和设置界面的颜色与状态。
- 更新安装包按下载源分别短测直连和当前代理并选择线路；仅作用于公共更新下载，不改变账号、Codex 任务或系统代理。
- Codex 大包支持最多四路受校验的分段下载、完成分段缓存与中断重试；服务器不支持可靠 Range/ETag 时回退单流。Navo 保留原有差分下载和安装验证，增加有限网络重试。
- 下载进度显示所用线路及单流/整包重试次数，保留官方包身份、版本、架构和安装后版本回读校验。不承诺固定倍数提速。

## 1.2.150

- 主界面和悬浮窗补齐工作空间 `self_serve_business_prolite` 套餐显示为 Business×5；不把其他 Business 套餐推定为 5 倍档位。
- 订阅查询优先使用账号独立网页的登录态，严格匹配当前工作空间，分别保存官方返回的到期日和续费日，不混用个人账号或其他工作空间的日期。
- 区分网页验证、网页登录失效、订阅访问被拒绝、工作空间未匹配等状态，并提供中英文提示；网页验证失败不再连续重试，后台重试降频，同账号并发查询合并。
- 已在用户完成网页验证后，用真实工作空间账号验证新版读取函数可返回套餐、到期日及续费日。网页验证仍需用户手动完成，不绕过验证。

## 1.2.149

- API Codex 在直接启动遇到 EPERM/EACCES 时，增加 Windows 系统激活回退；普通账号共用带启动参数的系统激活方式，保留代理、语言和语言桥接参数。
- 普通账号启动临时强制 ChatGPT 登录与文件凭证，覆盖残留 API provider/登录模式并过滤继承的 API 环境变量；不覆盖原账号凭证。授权回写仅接受同一账号的 ChatGPT 凭证，阻止 API Key 或其他账号覆盖。
- 系统激活时为任务子进程补充本次代理配置；原配置随启动事务恢复，不修改系统全局代理。API 启动事务仍在进行时，后台检测不提前恢复配置；可选语言桥接失败不再误报整个 API 启动失败。

## 1.2.148

- CLI 运行依赖面板新增“安装并使用稳定版 CLI”，从 OpenAI 官方发布下载匹配架构的稳定版本，显示下载与安装阶段进度，支持下载取消和失败重试。
- CLI 安装复用后台代理，校验文件大小、SHA-256、稳定版本及隔离启动握手；通过后才启用，失败不替换原配置，不覆盖桌面端、账号凭证或会话。
- 稳定版优先；无可用稳定版时，预发行版须通过临时空目录中的 app-server 握手才能作为备用。自定义路径使用相同验证规则。
- 额度与模型读取使用 app-server 默认标准输入输出通道，移除旧的 `--stdio` 参数；保持原有账号目录、凭证、代理和唤醒流程。
- 完善 CLI 输入框样式及安装状态中英适配。

## 1.2.147

- Codex CLI 运行依赖新增候选来源、版本校验与失败明细，支持重新检测和显式路径；检测采用异步校验、缓存和独立时限，慢来源不会丢弃已找到的候选。
- 额度读取统一请求归属，覆盖后台、主界面、浮窗、健康检查、唤醒和账号池刷新，完善并发、截止与退避处理；结果未确认的提交不自动重发。
- 分离更新缓存读取与主动检查，完善线路切换、取消和显式安装状态；安装包探测在 HEAD 返回 405/501 时支持有界 Range GET 回退。
- 调整账号卡片、网络列表、通知声音控件和启动弹窗布局；浮窗按内容限高并支持小屏滚动，区分额度读取时间、状态快照时间与最近活动根任务，补齐动态中英文提示。
- 历史优化接入活动文件保护、独立备份、空间预算和哈希校验；备份信息可查看，读取失败保留提示并支持重试。
- 修复模型目录跨目标混用、重置卡刷新状态和新 Key 成员选择；改善项目归属合并与用量游标索引，保留异常恢复，不改变统计和计价规则。
- 隔离回归 614/614 通过，最终浮窗聚焦回归 13/13 通过；完成假数据界面检查及候选包内容、版本和更新产物校验。未现场执行真实账号请求、重置卡兑换、唤醒、更新安装或跨账号切换；另一台电脑的 CLI 问题仍需该电脑的诊断明细。

## 1.2.146

- 卡片操作区改为等宽自适应排列，统一按钮间距，避免新增模型检测按钮后出现右侧留空或换行不齐。
- 卡片模式将删除按钮排列在最右侧；普通账号和 API 卡片共用排列规则，列表模式保持不变。

## 1.2.145

- 模型检测入口改为烧瓶图标，与悬浮窗入口区分。
- 模型检测新增“包含使用中的账号”选项，默认关闭；勾选后确认提示说明与当前任务共享额度及限流。
- 悬浮窗普通账号标签改为显示已识别套餐。只有返回套餐名明确包含倍数时显示 Pro×5 / Pro×20，不根据额度推测档位。
- 包含 1.2.144 的展开布局、悬停提示和重置卡中英文修复。

## 1.2.144

- 修复模型检测展开账号后复选框占满行宽、账号文字被挤成竖排的问题；分类改为纵向折叠列表，账号和模型选项使用独立文字布局。
- 修复模型检测入口显示空白悬停提示的问题。
- 重置卡常见官方标题、状态、范围和赠送说明按应用中英文设置显示，到期时间使用本地日期格式；未知官方说明保留原文，不猜测翻译。

## 1.2.143

- 新增普通账号、临时账号和 API Codex 模型检测，区分模型列表与实际生成检测，显示限流、额度、权限和网络等失败类型；支持取消和失败项重试。
- 重置卡支持查看详情及确认使用；操作记录持久化，超时重试复用原操作，避免重复扣卡。
- 临时账号尝试通过已有凭证读取套餐到期信息；上游未返回时明确显示读取状态，不猜测套餐或到期日。
- 卡片模式使用紧凑图标操作按钮，保留悬停提示；完善中英文文案和费用不完整提示。
- API 用量新增逐请求模型与计价记录、重复请求去重；历史费用仅在记录完整且数量核对一致时重算，缺少模型明细的旧统计不会按当前模型猜算。
- 修复模型检测按请求模型而非实际返回模型计费，以及相同 Token 数量检测请求被误去重的问题。包含此前 GPT-6 Astra 价格匹配和更新流程优化。
- 安装包未进行代码签名；本次验证不实际消耗用户重置卡。

## 1.2.142

- 历史费用重算遇到缺失记录或失败时逐步延长重试间隔至最多 24 小时，避免反复读取大文件；重算使用固定游标快照。
- 两套更新增加本地阶段耗时记录，采用字段白名单，不记录账号、Key、代理名称、URL 或原始报错。进度记录限频并轮转保留一份旧日志。
- 包含 1.2.141 的更新流程、后台刷新和 UI 优化。

## 1.2.141

- 更新检查区分网络错误、缺失发布清单和确实尚未提供的安装包，避免错误显示“最新版本”或“分发中”。
- Codex 下载新增速度、已下载大小、预计剩余时间、取消和有条件断点续传；完整缓存经哈希校验后复用，安装失败可重试而不必重复下载。
- 隔离两套更新的网络会话，减少重复代理配置；官方直包可用时跳过 Store 探测，明确失败时缩短无效等待；下载停滞会超时报错。
- Codex 直包完成下载及元数据检查后再请求关闭正在运行的应用；并发点击共享同一次安装操作，更新状态采用原子写入。
- 后台额度刷新限制并发并优先处理当前账号；套餐到期查询限制并发并复用有效缓存。
- 模型列表支持分页，并优先选择较新的稳定版 CLI；调整新模型费用估值及创建 Key 对话框样式。
- 历史费用重算仅更新可完整核对的统计桶，并保留备份；不根据用户手动使用额度的变化推断自动唤醒成功。
- 发布流水线校验版本标签、安装包、更新清单和 blockmap 一致性后再上传产物。当前版本仍未进行代码签名。
...
- Kept live API proxy switching: changing a route updates the stable relay target without changing the endpoint already held by Codex or forcing another firewall setup.
- Preserved Windows sandbox, plugin, and other defaults generated by Codex during a first API launch instead of deleting the complete generated `config.toml`; launch-only model, provider, and bearer-token entries are still removed on restore.
- Added regression coverage for stable proxy forwarding, reserved proxy ports, live route switching, and first-run configuration restoration.

## 1.2.119

- Added OpenAI-compatible non-streaming Responses and Chat Completions aggregation while continuing to use the account-backed streaming upstream internally.
- Reconstructed complete `output`, `output_text`, and `choices[0].message.content` fields from SSE delta/done events, and enriched `response.completed` for streaming clients.
- Accepted `max_output_tokens` without forwarding the unsupported field to the account-backed Codex endpoint; the requested value is retained in normalized Responses results.
- Added SSE heartbeats, continuous sequence repair, upstream cancellation on client disconnect, and stable `x-request-id` response headers.
- Added account-pool cooldown and categorized errors plus the authenticated `/v1/pool/health` diagnostics endpoint.
- Synchronized generated task plans, progress updates, and status explanations with the Simplified Chinese or English language selected for managed Codex launches.
- Included the pending API Codex startup, session-state restoration, and proxy protocol compatibility fixes from the 1.2.114-1.2.118 development builds.
